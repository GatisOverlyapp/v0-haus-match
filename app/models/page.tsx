...
const getPriceRanges = () => {
  const ranges = new Set(houseModels.map((m) => m.price_range))
...
export default function ModelsPage() {
  ...
  const filteredAndSortedModels = useMemo(() => {
    ...
    return sorted
  }, [category, priceRange, bedrooms, sizeMin, sizeMax, sort])

  const categories = getUniqueCategories()
  const priceRanges = getPriceRanges()

  // Get size range options
  const allSizes = houseModels.map((m) => m.size_sqm)
  ...
  const hasActiveFilters = category || priceRange || bedrooms || sizeMin || sizeMax
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-gradient-to-r from-teal-600 to-teal-700 text-white py-12 md:py-16">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">All Models</h1>
          <p className="text-lg text-teal-100 max-w-2xl">
            Browse all prefab home models from our trusted manufacturers. Filter and sort to find your perfect match.
          </p>
        </div>
      </header>
